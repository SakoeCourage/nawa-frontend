...
function Blankimageplaceholder({onImageUpload, isDragging,dragProps}){
  return <div onClick={onImageUpload} {...dragProps} className={`max-w-sm mx-auto w-full  cursor-pointer ${isDragging && 'opacity-50'}`}>
  <nav className="border-2 rounded-md overflow-hidden flex items-center justify-center h-full w-full  aspect-square  ">
      <nav className={`flex flex-col items-center gap-2 ${isDragging && ' pointer-events-none'}`}>
          ...
          <span className="font-medium">
              {isDragging ? 'Drop image here' : 'Click or drop images here'}
          </span>
      </nav>
  </nav>
...
function Imageuploadcomponent({ getImages, imgs, maxNumber = 5, maxFileSize = 10485760, onError }) {
    ...
    const mapErrorMessage = {
        maxFileSize: ' One or more image Exceeded recomended size',
    ...
    return (
        <ImageUploading
            ...
            onError={(err) => onError(mapErrorMessage[Object.keys(err)])}
            dataURLKey="data_url"
            acceptType={["jpg", 'png']}
        >
            {({
                ...
                <div className="">
                    {!images.length &&<Blankimageplaceholder isDragging={isDragging} onImageUpload={onImageUpload} dragProps={dragProps}/> }
                    <div className=' grid grid-cols-2 lg:grid-cols-3 gap-5'>
                        {imageList.map((image, index) => (
                            <div key={index} className=" aspect-square w-full h-full object-cover relative">
                                {/* eslint-disable-next-line @next/next/no-img-element */}
                                <img src={image.data_url} alt="" width="" className=' w-full h-full aspect-square' />
                                <div className="absolute top-1 right-1">
                                    <button onClick={() => onImageRemove(index)}>
                                        <svg className=' text-red-700' width="24" height="24" viewBox="0 0 24 24" fill="#ff0000" xmlns="http://www.w3.org/2000/svg">
                          ...
                          {Boolean(images.length) && !(Number(images.length) === maxNumber)  && <Blankimageplaceholder isDragging={isDragging} onImageUpload={onImageUpload} dragProps={dragProps}/>}
                    </div>
